...
const EnterDetails = ({ route, navigation }) => {
  ...
  const [status, setStatus] = useState('accepted'); //accepted | rejected | pending | ticket
  ...
  let addCertificate = () => {
    ...
    db.transaction(function (tx) {
      tx.executeSql(
        'INSERT INTO certificate_table (student_ref, teacher_ref, certificate_name, certificate_uri, certificate_venue, certificate_days, certificate_date, certificate_type, certificate_subtype, certificate_status, certificate_points) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
        [parseInt(student_id), parseInt(teacher_id), name, pickedImagePath, venue, days, mydate.toLocaleDateString("en-US"), value1, value3, status, parseInt(points)],
        (tx, results) => {
